                ...
                for (RegiaoModel regiao : regioes) { %>
                    <div class="card-localidade">
                        <div class="localidade-image">
                            <img src="<%= request.getContextPath() %>/images/regioes/placeholder.jpg"
                                 alt="<%= regiao.getNomeRegiao() %>">
                        </div>
                        <div class="localidade-info">
                            <h2><%= regiao.getNomeRegiao() %></h2>
                            <div class="localidade-stats">
                                 <p>👤 <%= regiao.getTotalArtistas() %> artista<%= regiao.getTotalArtistas() != 1 ? "s" : "" %></p>
                                 <p>🎨 <%= regiao.getTotalObras() %> obra<%= regiao.getTotalObras() != 1 ? "s" : "" %></p>
                            </div>
                            <a href="<%= request.getContextPath() %>/explorar?regiao=<%= regiao.getIdRegiao() %>"
                               class="btn-primary">Explorar Região</a>
                        </div>
                    </div>
            <%  }
            } else { %>
