...
public class UsuarioDAO {
    ...
    public boolean inserir(UsuarioModel usuario) {
        ...
        try {
            ...
            // visitante não pode ter portfolio/categoria/tags (check constraint)
    ...
    public List<UsuarioModel> listarTodos() {
        ...
        try {
            ...
            while (rs.next()) usuarios.add(extrairUsuario(rs));
    ...
    public List<UsuarioModel> listarArtistas() {
        ...
        try {
            ...
            while (rs.next()) artistas.add(extrairUsuario(rs));
    ...
    private UsuarioModel extrairUsuario(ResultSet rs) throws SQLException {
        ...
        int categoriaPrincipal = rs.getInt("categoria_principal");
        if (!rs.wasNull()) usuario.setCategoriaPrincipal(categoriaPrincipal);

        usuario.setTagsPrincipais(rs.getString("tags_principais"));

        int idRegiao = rs.getInt("id_regiao");
        if (!rs.wasNull()) usuario.setIdRegiao(idRegiao);

        Timestamp timestamp = rs.getTimestamp("data_criacao");
        if (timestamp != null) usuario.setDataCriacao(timestamp.toLocalDateTime());
